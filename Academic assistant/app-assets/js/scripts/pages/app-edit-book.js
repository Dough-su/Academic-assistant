...
window.onload = function () {
    ...
    if (id != "../upload/null.pdf") {
        ...
        $.ajax({
            ...
            data: { "meth": "book", userid: document.cookie.split("=")[0], "paperid": paperid },
            dataType: "json",
            success: function (data) {
                console.log(data);
                $("input[name='title']").val(data[0].bookname)
                $("input[name='issn_no']").val(data[0].Publicationnumber)
                $("select[name='jurnal_type']").val(data[0].identity)
                $("select[name='my_rank']").val(data[0].ranking)
                $("input[name='press']").val(data[0].publishinghouse)
                $("select[name='textbook']").val(data[0].booktype)
                $("input[name='word_count']").val(data[0].wordcount)
                $("input[name='pub_location']").val(data[0].publisherplace)
                $("textarea[name='description']").val(data[0].Remarks)
                $("input[class='mtr-input years']").val(data[0].Publisheddate.split(" ")[0].split("/")[0])
                $("input[class='mtr-input months']").val(data[0].Publisheddate.split(" ")[0].split("/")[1])
                $("input[class='mtr-input dates']").val(data[0].Publisheddate.split(" ")[0].split("/")[2])
...
function uploaddetail(timestamp,draft) {
        var book = {
           ...
           Publicationnumber:$("input[name='issn_no']").val(),
           identity:$("select[name='jurnal_type']").val(),
           ranking:$("select[name='my_rank']").val(),
           publishinghouse:$("input[name='press']").val(),
           booktype:$("select[name='textbook']").val(),
           wordcount:$("input[name='word_count']").val(),
           publisherplace:$("input[name='pub_location']").val(),
           Publisheddate:$("input[class='mtr-input years']").val()+"-"+$("#mtr-datepicker-0-input-months > div.mtr-content > input").val()+"-"+$("#mtr-datepicker-0-input-dates > div.mtr-content > input").val(),
           Remarks:$("textarea[name='description']").val(),
           draft:draft
        }
        if (id != "../upload/null.pdf") {
            book.meth="bookupdate";
    }
                console.log(book);
        $.ajax({
