...
window.onload = function () {
    ...
    if (id != "../upload/null.pdf") {
        ...
        $.ajax({
            ...
            data: { "meth": "software", userid: document.cookie.split("=")[0], "paperid": paperid },
            dataType: "json",
            success: function (data) {
                console.log(data);
                    //开始填充元素
                    $("input[name='title']").val(data[0].Copyrightname)
                    $("input[name='certificate_no']").val(data[0].CertificateNo)
                    $("select[name='my_rank']").val(data[0].ranking)     
                    $("input[name='register_no']").val(data[0].licensenumber)
                    $("input[value='"+data[0].Copyrighttype+"']").attr("checked", "checked")
                    $("input[name='copyright_owner']").val(data[0].copyrightowner)
                    $("textarea[name='description']").val(data[0].Remarks)
                    $("input[class='mtr-input years']").val(data[0].Developmentcompletiontime.split(" ")[0].split("/")[0])
                    $("input[class='mtr-input months']").val(data[0].Developmentcompletiontime.split(" ")[0].split("/")[1])
                    $("input[class='mtr-input dates']").val(data[0].Developmentcompletiontime.split(" ")[0].split("/")[2])
                    $("#mtr-datepicker-1-input-years > div.mtr-content > input").val(data[0].Gettime.split(" ")[0].split("/")[0])
                    $("#mtr-datepicker-1-input-months > div.mtr-content > input").val(data[0].Gettime.split(" ")[0].split("/")[1])
                    $("#mtr-datepicker-1-input-dates > div.mtr-content > input").val(data[0].Gettime.split(" ")[0].split("/")[2])
...
function uploaddetail(timestamp,draft) {
        var software = {
           ...
           paperid:timestamp,
           Copyrightname:$("input[name='title']").val(),
           CertificateNo:$("input[name='certificate_no']").val(),
           ranking:$("select[name='my_rank']").val(),
           Developmentcompletiontime:$("input[class='mtr-input years']").val()+"-"+$("#mtr-datepicker-0-input-months > div.mtr-content > input").val()+"-"+$("input[class='mtr-input dates']").val(),
           Gettime:$("#mtr-datepicker-1-input-years > div.mtr-content > input").val()+"-"+$("#mtr-datepicker-1-input-months > div.mtr-content > input").val()+"-"+$("#mtr-datepicker-1-input-dates > div.mtr-content > input").val(),
           licensenumber:$("input[name='register_no']").val(),
           Copyrighttype:$("input[name='copyright_type']:checked").val(),
           copyrightowner:$("input[name='copyright_owner']").val(),
           Remarks:$("textarea[name='description']").val(),
           draft:draft
        }
        if (id != "../upload/null.pdf") {
            software.meth="softwareupdate";
    }
