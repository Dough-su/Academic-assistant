...
window.onload = function () {
    ...
    if (id != "../upload/null.pdf") {
        ...
        $.ajax({
            ...
            data: { "meth": "certificate", userid: document.cookie.split("=")[0], "paperid": paperid },
            dataType: "json",
            success: function (data) {
                console.log(data);
                $("input[name='title']").val(data[0].filetitle)
                $("input[value='"+data[0].Certificatetype+"']:checked").prop("checked",true)
                $("input[name='cert_no']").val(data[0].CertificateNo)
                $("input[name='cert_authorization']").val(data[0].grantingagency)
                $("textarea[name='description']").val(data[0].Remarks)
                $("input[class='mtr-input years']").val(data[0].Gettime.split(" ")[0].split("/")[0])
                $("input[class='mtr-input months']").val(data[0].Gettime.split(" ")[0].split("/")[1])
                $("input[class='mtr-input dates']").val(data[0].Gettime.split(" ")[0].split("/")[2])
...
function uploaddetail(timestamp,draft) {
        var certificate = {
           ...
           Certificatetype:$("input[name='cert_type']:checked").val(),
           Gettime:$("#mtr-datepicker-0-input-years > div.mtr-content > input").val()+"-"+$("#mtr-datepicker-0-input-months > div.mtr-content > input").val()+"-"+$("#mtr-datepicker-0-input-dates > div.mtr-content > input").val(),
           CertificateNo:$("input[name='cert_no']").val(),
           grantingagency:$("input[name='cert_authorization']").val(),
           Remarks:$("textarea[name='description']").val(),
           draft:draft
        }
        if (id != "../upload/null.pdf") {
            certificate.meth="certificateupdate";
    }
