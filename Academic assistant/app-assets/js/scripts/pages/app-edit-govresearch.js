...
window.onload = function () {
    ...
    if (id != "../upload/null.pdf") {
        ...
        $.ajax({
            ...
            data: { "meth": "govresearch", userid: document.cookie.split("=")[0], "paperid": paperid },
            dataType: "json",
            success: function (data) {
                console.log(data);
                    //开始填充元素
                    $("input[name='title']").val(data[0].Subjectname)
                    $("input[name='source']").val(data[0].Fundname)
                    $("select[name='typeid']").val(data[0].Subjecttype)
                    $("input[name='organization']").val(data[0].Grantingunit)
                    $("select[name='lever']").val(data[0].Subjectlevel)
                    $("input[name='number']").val(data[0].subjectnumber)
                    $("input[name='funds']").val(data[0].Receiptofexpenses)
                    ...
                    $("#mtr-datepicker-1-input-years > div.mtr-content > input").val(data[0].deadline.split(" ")[0].split("/")[0])
                    $("#mtr-datepicker-1-input-months > div.mtr-content > input").val(data[0].deadline.split(" ")[0].split("/")[1])
                    $("#mtr-datepicker-1-input-dates > div.mtr-content > input").val(data[0].deadline.split(" ")[0].split("/")[2])
                    $("select[name='kc_status']").val(data[0].subjectstatus)
...
function uploaddetail(timestamp,draft) {
        var govresearch = {
           ...
           paperid:timestamp,
           Subjectname:$("input[name='title']").val(),
           Fundname:$("input[name='source']").val(),
           Subjecttype:$("select[name='typeid']").val(),
           Grantingunit:$("input[name='organization']").val(),
           Subjectlevel:$("select[name='lever']").val(),
           subjectnumber:$("input[name='number']").val(),
           Receiptofexpenses:$("input[name='funds']").val(),
           ...
           deadline:$("#mtr-datepicker-1-input-years > div.mtr-content > input").val()+"-"+$("#mtr-datepicker-1-input-months > div.mtr-content > input").val()+"-"+$("#mtr-datepicker-1-input-dates > div.mtr-content > input").val(),
           subjectstatus:$("select[name='kc_status']").val(),
           Remarks:$("textarea[name='description']").val(),
           draft:draft
        }
        if (id != "../upload/null.pdf") {
        govresearch.meth="govresearchupdate";
    }
                console.log(govresearch);
        $.ajax({
