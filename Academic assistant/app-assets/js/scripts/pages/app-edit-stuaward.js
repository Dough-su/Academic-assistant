...
window.onload = function () {
    ...
    if (id != "../upload/null.pdf") {
        ...
        $.ajax({
            ...
            data: { "meth": "stuaward", userid: document.cookie.split("=")[0], "paperid": paperid },
            dataType: "json",
            success: function (data) {
                console.log(data);
                    //开始填充元素

                    $("input[name='title']").val(data[0].gamename)
                    $("select[name='my_rank']").val(data[0].ranking)
                    $("input[name='students']").val(data[0].Awardwinningstudents)
                    $("select[name='ranking']").val(data[0].Awards)
                    $("#filter_lever > div.col-sm-3.select > select").val(data[0].level)
                    $("input[name='school']").val(data[0].Grantingunit)
                    $("textarea[name='description']").val(data[0].Remarks)
                    $("input[class='mtr-input years']").val(data[0].winningtime.split(" ")[0].split("/")[0])
                    $("input[class='mtr-input months']").val(data[0].winningtime.split(" ")[0].split("/")[1])
                    $("input[class='mtr-input dates']").val(data[0].winningtime.split(" ")[0].split("/")[2])
...
function uploaddetail(timestamp,draft) {
        var stuaward = {
           ...
           paperid:timestamp,
           gamename:$("input[name='title']").val(),
           Awardwinningstudents:$("input[name='students']").val(),
           ranking:$("select[name='my_rank']").val(),
           Awards:$("select[name='ranking']").val(),
           level:$("#filter_lever > div.col-sm-3.select > select").val(),
           Grantingunit:$("input[name='school']").val(),
           winningtime:$("input[class='mtr-input years']").val()+"-"+$("#mtr-datepicker-0-input-months > div.mtr-content > input").val()+"-"+$("#mtr-datepicker-0-input-dates > div.mtr-content > input").val(),
           Remarks:$("textarea[name='description']").val(),
           draft:draft
        }
        if (id != "../upload/null.pdf") {
            stuaward.meth="stuawardupdate";
    }
                console.log(stuaward);
        $.ajax({
