...
window.onload = function () {
    ...
    if (id != "../upload/null.pdf") {
        ...
        $.ajax({
            ...
            data: { "meth": "award", userid: document.cookie.split("=")[0], "paperid": paperid },
            dataType: "json",
            success: function (data) {
                console.log(data);
                    //开始填充元素
                    $("input[name='title']").val(data[0].Honoraryname)
                    $("select[name='my_rank']").val(data[0].ranking)
                    $("select[name='rank']").val(data[0].Awards)
...
function uploaddetail(timestamp,draft) {
        var award = {
           ...
           paperid:timestamp,
           Honoraryname:$("input[name='title']").val(),
           ranking:$("select[name='my_rank']").val(),
           Awards:$("select[name='rank']").val(),
