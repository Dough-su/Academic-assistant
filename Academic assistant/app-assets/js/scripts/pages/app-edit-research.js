...
window.onload = function () {
    ...
    if (id != "../upload/null.pdf") {
        ...
        $.ajax({
            ...
            data: { "meth": "research", userid: document.cookie.split("=")[0], "paperid": paperid },
            dataType: "json",
            success: function (data) {
                console.log(data);
                    //开始填充元素
                    $("input[name='title']").val(data[0].Subjectname);
                    $("input[name='source']").val(data[0].contractsource);
                    $("input[name='number']").val(data[0].ContractNumber);
...
function uploaddetail(timestamp,draft) {
        var research = {
           ...
           contractsource:$("input[name='source']").val(),
           ContractNumber:$("input[name='number']").val(),
